...
- JS option keys (canonical): `checkinUrl`, `checkInInterval`, `ticketMaxLength`, `successAudioUrl`, `failAudioUrl`, `textSuccessClass`, `textWarningClass`, plus the 6 i18n message keys (`MOD_EVENT_QRSCAN_ERROR_NO_CAMERA`, `_CAMERA_BUSY`, `_OFFLINE`, `_INVALID_QR`, `_EB_ABSENT`, `_UNKNOWN`) — full list authoritative in `contracts/module-params.md`.
...
6. **FR-6 Response Display**: Single Bootstrap 5 modal (`qrscanModal`) shows text-escaped `message`; Close via `data-bs-dismiss`. If `bootstrap.Modal` is unavailable, render the message as inline text instead (FR-7 error path; modal and inline fallback are never shown simultaneously). Body styling uses module params `text_success_class` (default `text-success`) and `text_warning_class` (default `text-danger`); no external CSS file (JS-set inline styles and embedded `<style>` in the template are allowed). The result pop-up must be fully visible without scrolling: its top must not render above the first HTML element of the module output (currently the `#reader` video element).
7. **FR-7 User Feedback**: All failures show human-readable modal/inline text (no `alert()`). Acceptance criteria: each negative case (no camera, camera in use, offline, invalid QR, EB absent, unknown) yields modal/inline text with the correct EB sound where applicable. Invalid ticket codes are detected by the check-in module, which returns its own message in the JSON response (no `bad_key` case in this module).
...
13. **FR-13 Dutch translation**: The module MUST ship a complete Dutch (nl-NL) translation covering the same key set as en-GB: aim instruction, Start, Stop, Switch camera, Scan Result title, Close, all six user-facing error messages (no camera, camera busy/in use, offline, invalid QR, EB absent with `%s`, unknown), the check-in-config notice, and backend labels (description, check-in interval, ticket max length, text classes — `layout` param removed per 2026-09-20 clarification, single `tmpl/default.php` only).
...
5. **SC-5 Error Handling [build-gate]**: Every error case displays a specific human-readable message and the correct EB sound where applicable: no camera → "No camera available."; camera in use → "Camera is in use by another application."; offline → "No network, check-in service unavailable"; invalid QR → "This seems not to be a ticket QR code"; EB absent → "Error while communicating with check-in service, error code is %s" (where `%s` is the HTTP status code). Invalid ticket codes are reported via the check-in module's own JSON `message` (no `bad_key` module message). These English texts are the **fallback** values (see FR-14/FR-16); the displayed text always resolves via language keys. Union of JS cases: `no_camera, camera_busy, offline, invalid_qr, eb_absent, unknown`. Verified by T16 negatives.
...
- **Language File**: Per-language ini pair (`nl-NL.mod_event_qrscan.ini` for site strings, `nl-NL.mod_event_qrscan.sys.ini` for installation labels), mirroring the en-GB key set (19 keys)
...
- Q: What type of administrator input should the new scan-speed setting use in the module settings form? → A: B — Number field with min 1, max 10, default 2 (originally specified as integer in error, corrected per T054 — the shipped manifest uses `type="number"`).
