...
1. Lint: `php -l` (entry + `Helper/`), `node --check js/site-checkin-default.js` (NOT vendored `js/html5-qrcode.min.js`), `uglifyjs js/site-checkin-default.js -o js/site-checkin-default.min.js`, `ET.parse` all XMLs, manifest required-field check
    (incl. `namespace` matches `Helper/EventQrscanHelper.php`).
...
3. `shutil.make_archive("mod_event_qrscan", "zip", tmp)` from
   `tmpl/, language/, Helper/, js/` (incl. vendored `js/html5-qrcode.min.js`) + `LICENSE` + manifest + `*.php`.
4. ZIP completeness assert (required entries present incl. `js/html5-qrcode.min.js`, no temp/template leak).
